...
Use the following query to determine the blank characters used by each of the installed
PGSQL Text Search Language configurations. Note, we range from 2 to the unicode limit of
55295, and interpret each of the characters in each of the installed languages.
We omit character 0001 as that is a bell character used in computed string and thus omitted.
The following query SHOULD return 64 rows (representing each of the 64 space characters), 
each with a count of 29 (representing the number of default languages in the system).
