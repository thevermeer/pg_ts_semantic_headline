...
We assume at this point that you are running a postgresql database, and while this code has been developed based on techniques available to PostgreSQL13 users, the contents of this repo were developed against the development version of PostgreSQL 17.
...
To do this, we are going to add a number of redundant columns to the table, in order to compared the performance of pre-realizing values within a search index, versus the cost of performing data transformations on the fly.

To do that we will create a table that contains:
- an ID field as a primary key
- `content` TEXT column to store the actual textual `content`
- `content_tsv` TSVECTOR column will store the pre-computed TSVector representation of the content column parsed against the default english-stem disctionary
- `content_exact_tsv` TSVECTOR is the space-tokenized exact match TS vector, created using the simple (instead of the default english-stem) dictionary.
- `indexed_content` TEXT is a version of `content` with some pre-processing done on special character to encourage better positional indexing. 
- `content_arr` TEXT[] is an array of words inside the `content` string. We store the array for faster text lookup and assembly of headlines.
...
## 2. Add `prepare_text_for_tsvector` function
In this repository, we derive a method for pre-processing a large volume of text, such that we can use positional information in TSVectors to quickly retrieve the content that a search query matches on. We will use this function to pre-process our content indices in the `files` table.
...
See [[Preparing source text for better positional indexing](https://github.com/thevermeer/postgresql_semantic_tsheadline/blob/main/problems/exact_matches.md#preparing-source-text-for-better-positional-indexing)] for the how we derived the `prepare_text_for_tsvector` function, which is also listed below


## 3. Create a trigger to compute tsvector and populate content_tsv
...
BEGIN
	NEW.indexed_content   := prepare_text_for_tsvector(NEW.content);
...
To emunlate large texts, we will fill the files table with a number of rows of content, with each row actually containing the first chapter to Charles Dickens' A tale of two cities. The below code will generate 100 rows with the 16,346 repeated once per row  (the `1` on second-last line), as the entry for the `content` column. The maximum number of words one can index in a TSVector is 16,383, so the purpose of adding this much text is to emulate performance across an index of large files.
